...
export function VueArchive() {
  const { items, reouvrirItem } = useGarage();
  ...
  const archives = items.filter(i => i.etat === 'termine');
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{
              background: '#6b7280', color: 'white', fontSize: 13,
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 16, color: typeColor }}>
            #{item.numero}
          </span>
          <span style={{ fontSize: 11, background: `${typeColor}18`, color: typeColor, padding: '2px 8px', borderRadius: 10, fontWeight: 600 }}>
            {typeLabel}
          </span>
          {item.urgence && (
      ...
      <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
        {item.stationsActives.slice(0, 6).map(stationId => (
          <div
            key={stationId}
            title={TOUTES_STATIONS_COMMUNES.find(s => s.id === stationId)?.label ?? stationId}
            style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e' }}
          />
        ))}
        {item.stationsActives.length > 6 && (
          <span style={{ fontSize: 10, color: '#9ca3af' }}>+{item.stationsActives.length - 6}</span>
        )}
      </div>
      ...
      <div style={{ flexShrink: 0 }} onClick={e => e.stopPropagation()}>
        {!isConfirming ? (
          <button
            onClick={(e) => { e.stopPropagation(); setConfirmerReouverture(item.id); }}
            style={{
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onReouvrir(); }}
            ...
            <button
              onClick={(e) => { e.stopPropagation(); setConfirmerReouverture(null); }}
...
function PanneauDetailArchive({ item, onClose, onReouvrir, onSupprimer }: {
  ...
  const dateCreation = new Date(item.dateCreation).toLocaleDateString('fr-CA', {
    ...
    <div style={{
      ...
      <div style={{ padding: 24 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 15, color: '#374151', fontWeight: 500, marginBottom: 12 }}>
            {item.label}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            <span style={{ fontSize: 12, background: '#dcfce7', color: '#166534', padding: '3px 10px', borderRadius: 4, fontWeight: 700 }}>
              ✓ ARCHIVÉ
            </span>
            {item.urgence && (
              <span style={{ fontSize: 12, background: '#fef3c7', color: '#92400e', padding: '3px 10px', borderRadius: 4, fontWeight: 600 }}>
                ⚡ URGENT
              </span>
            )}
          </div>
        ...
        {/* Dates */}
        <div style={{
          background: '#f8fafc', borderRadius: 8, padding: '12px 14px', marginBottom: 20,
          display: 'flex', flexDirection: 'column', gap: 6,
        }}>
          <div style={{ fontSize: 13, color: '#6b7280' }}>
            <span style={{ fontWeight: 600, color: '#374151' }}>Créé le : </span>{dateCreation}
          </div>
          {dateArchive && (
            <div style={{ fontSize: 13, color: '#6b7280' }}>
              <span style={{ fontWeight: 600, color: '#374151' }}>Livré le : </span>{dateArchive}
            </div>
          )}
        </div>
          ...
          {item.stationsActives.map(stationId => {
            ...
            return (
              <div key={stationId} style={{
                ...
                background: statut === 'termine' ? '#f0fdf4' : 'white',
              }}>
                <div style={{ fontSize: 14, fontWeight: statut === 'termine' ? 600 : 400, color: '#374151' }}>
                  {station?.label ?? stationId}
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color: statusColor, background: `${statusColor}18`, padding: '2px 8px', borderRadius: 4 }}>
                  {statusLabel}
                </span>
              </div>
            );
        ...
        {item.notes && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 13, color: '#6b7280', background: '#f8fafc', borderRadius: 8, padding: '10px 12px', lineHeight: 1.6 }}>
              {item.notes}
            </div>
          </div>
        )}
        ...
        <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {!confirmerReouverture ? (
            <button
              onClick={() => setConfirmerReouverture(true)}
              style={{
            ...
            <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#c2410c', marginBottom: 6 }}>
                ⚠️ Confirmer la réouverture?
              </div>
              <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 14 }}>
                Le job sera remis en attente avec toutes les étapes à refaire.
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  onClick={() => setConfirmerReouverture(false)}
          ...
          {isGestion && (
            !confirmerSuppression ? (
              ...
              <div style={{ background: '#fff5f5', border: '1px solid #fca5a5', borderRadius: 8, padding: 14 }}>
                ...
                <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 14 }}>
                  <strong>#{item.numero} — {item.label}</strong><br />
