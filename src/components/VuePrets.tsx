...
export function VuePrets() {
  ...
  const prets = items.filter(i => i.etat !== 'termine' && toutesEtapesCompletees(i));

  const filtres = prets.filter(i => filtreType === 'tous' || i.type === filtreType)
    .sort((a, b) => {
      // Prêts à livrer en premier (avec date)
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#f8fafc', overflow: 'hidden' }}>
      <div style={{
        ...
        marginRight: selectedItem ? 400 : 0, transition: 'margin-right 0.3s ease',
      }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            {aVendre > 0 && (
              <span style={{ background: '#f59e0b', color: 'white', fontSize: 12, fontWeight: 700, padding: '2px 10px', borderRadius: 12 }}>
                🏷️ {aVendre} à vendre
              </span>
            )}
            {aLivrer > 0 && (
              <span style={{ background: '#3b82f6', color: 'white', fontSize: 12, fontWeight: 700, padding: '2px 10px', borderRadius: 12 }}>
                🚛 {aLivrer} à livrer
              </span>
            )}
          </div>
          ...
          {([
            ...
            <button key={f.id} onClick={() => setFiltreType(f.id)}
              style={{
                ...
                background: filtreType === f.id ? '#22c55e' : 'white',
                color: filtreType === f.id ? 'white' : '#6b7280',
                fontWeight: filtreType === f.id ? 700 : 400,
              }}>
              {f.label}
              <span style={{ marginLeft: 6, fontSize: 11, opacity: 0.8 }}>
                {prets.filter(i => f.id === 'tous' || i.type === f.id).length}
              </span>
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
          {filtres.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 0', color: '#9ca3af' }}>
              ...
              <div style={{ fontSize: 14 }}>Les véhicules dont toutes les étapes sont complétées apparaîtront ici</div>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: 28, flexShrink: 0 }}>
                      {item.type === 'eau' ? <EauIcon /> : item.type === 'client' ? '🔧' : '🏷️'}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                        ...
                        <span style={{ fontSize: 11, background: statut.bg, color: statut.color, padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>
                          {statut.label}
                        </span>
                        {item.clientAcheteur && (
                          <span style={{ fontSize: 11, color: '#6b7280' }}>— {item.clientAcheteur}</span>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#9ca3af' }}>
                        ...
                        {item.dateLivraisonPlanifiee && (
                          <span style={{ color: '#1d4ed8', fontWeight: 600 }}>
                            📅 Livraison : {new Date(item.dateLivraisonPlanifiee).toLocaleDateString('fr-CA')}
                          </span>
                        )}
                      </div>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); archiverItem(item.id); }}
                      style={{
      ...
      <div style={{ padding: 24 }}>
          ...
          <div style={{ marginBottom: 20, padding: '14px', borderRadius: 10, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
              {([
                ...
                <button key={val}
                  onClick={() => onMettreAJour(item.id, { etatCommercial: val })}
                  style={{
            ...
            {(etatCommercial === 'reserve' || etatCommercial === 'vendu') && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <input
                  type="text"
                  value={item.clientAcheteur ?? ''}
                  onChange={e => onMettreAJour(item.id, { clientAcheteur: e.target.value || undefined })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => onMettreAJour(item.id, { dateLivraisonPlanifiee: e.target.value || undefined })}
                    style={{ width: '100%', padding: '8px 10px', borderRadius: 6, border: '1px solid #d1d5db', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                  />
                </div>
              </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 13, color: '#6b7280', lineHeight: 2 }}>
            ...
            {item.dateLivraisonPlanifiee && (
              <div><span style={{ fontWeight: 600, color: '#374151' }}>Livraison planifiée :</span> {new Date(item.dateLivraisonPlanifiee).toLocaleDateString('fr-CA')}</div>
            )}
          </div>
        </div>
        ...
        <button onClick={onArchiver}
          style={{ width: '100%', padding: '12px', borderRadius: 8, border: 'none', background: '#22c55e', color: 'white', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
          🏁 Livrer & Archiver
        </button>
      </div>
    </div>
